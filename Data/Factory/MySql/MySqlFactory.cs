...
namespace TLF.Data.Factory.Mysql
{
    ...
    public class MySqlFactory
    {
        ...
        string[] paramListTmp = null;
        string[] outParamTmp = null;
        ...
        public DataSet NTx_ExecuteDataSet(string command, string cmdType, string[] paramList, object[] valueList, string[] outParam, ref object[] outValue)
        {
            ...
            if (paramList != null)
            {
                paramListTmp = new string[paramList.Length];

                for (int idx = 0; idx < paramList.Length; idx++)
                {
                    if (valueList[idx] != null && paramList[idx] != null)
                    {
                        if (paramList[idx].IndexOf('@') > 0)
                        {
                            paramListTmp[idx] = paramList[idx].Replace("@", "p_");
                        }
                        else
                        {
                            paramListTmp[idx] = paramList[idx];
                        }

                        dbCommand.Parameters.Add(paramListTmp[idx].Trim().Replace("\t", ""), GetParameterDbType(valueList[idx]));
                        dbCommand.Parameters[paramListTmp[idx]].Direction = ParameterDirection.Input;
                        dbCommand.Parameters[paramListTmp[idx]].Value = valueList[idx];
                    }
                }
            }
            ...
            if (outParam != null)
            {
                outParamTmp = new string[outParam.Length];

                for (int idx = 0; idx < outParam.Length; idx++)
                {
                    if (outParam[idx].IndexOf('@') > 0)
                    {
                        outParamTmp[idx] = outParam[idx].Replace("@", "p_");
                    }
                    else
                    {
                        outParamTmp[idx] = outParam[idx];
                    }

                    dbCommand.Parameters.Add(outParamTmp[idx].Trim().Replace("\t", ""), GetParameterDbType(outValue[idx]));
                    dbCommand.Parameters[outParamTmp[idx]].Value = outValue[idx];
                }
            }
            ...
            if (outParam != null)
            {
                for (int idx = 0; idx < outParam.Length; idx++)
                {
                    outValue[idx] = dbCommand.Parameters[outParamTmp[idx]].Value.ToString();
                }
            }
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (paramList != null)
                    {
                        ...
                        for (int idx = 0; idx < paramList.Length; idx++)
                        {
                            if (paramList[idx] != null && paramList[idx].Length != 0)
                            {
                                ...
                                dbCommand.Parameters.Add(paramListTmp[idx].Trim().Replace("\t", ""), GetParameterDbType(dt.Columns[paramListTmp[idx].Replace("p_", "").Trim()]));
                                //if (paramList[idx].Replace("p_", "").Trim() == "UserName") {
                                //    MessageBox.Show("나는 " + dr[paramList[idx].Replace("p_", "").Trim()] +  " 입니다.");
                                //}
                                dbCommand.Parameters[paramListTmp[idx].Trim().Replace("\t", "")].Direction = ParameterDirection.Input;
                                ...
                                dbCommand.Parameters[paramListTmp[idx].Trim().Replace("\t", "")].Value = dr[paramListTmp[idx].Replace("p_", "").Trim()];
                    ...
                    if (outParam != null)
                    {
                        ...
                        for (int idx = 0; idx < outParam.Length; idx++)
                        {
                            ...
                            dbCommand.Parameters.Add(outParamTmp[idx].Trim().Replace("\t", ""), GetParameterDbType(outValue[idx]));
                            dbCommand.Parameters[outParamTmp[idx]].Direction = ParameterDirection.Output;
                        }
                    }
